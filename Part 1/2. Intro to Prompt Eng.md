...
graph TD
  ...
  D --> E[Fill-in-the-Middle (FIM) technique]
...
graph TD
  A[User prompt]
  A --> B[Toxicity Filtering]
  B --> C[Detect hate speech & inappropriate content]
  B --> D[Filter personal data]
  C --> E[Block harmful or offensive input]
  D --> F[Protect privacy & data security]
  E --> G[Safe prompt for code generation]
  F --> G
```
